...
def obtainRandomXORTrainingValues():
    ...
    if trainingDataSetNum > 3:
        return (1,1,0,1,3)
    elif trainingDataSetNum > 2:
        return (1,0,1,0,2)
    elif trainingDataSetNum > 1:
        return (0,1,1,0,1)
    else:
        return (0,0,0,1,0)
...
def computeSingleFeedforwardPass(alpha, inputDataList, wWeightArray, vWeightArray, biasHiddenWeightArray, biasOutputWeightArray):
    input0, input1 = inputDataList
    
    # Calculate hidden layer activations
    hiddenInput0 = wWeightArray[0,0]*input0 + wWeightArray[0,1]*input1 + biasHiddenWeightArray[0]
    hiddenInput1 = wWeightArray[1,0]*input0 + wWeightArray[1,1]*input1 + biasHiddenWeightArray[1]
    
    hiddenOutput0 = computeTransferFnctn(hiddenInput0, alpha)
    hiddenOutput1 = computeTransferFnctn(hiddenInput1, alpha)
    
    # Calculate output layer activations
    outputInput0 = vWeightArray[0,0]*hiddenOutput0 + vWeightArray[0,1]*hiddenOutput1 + biasOutputWeightArray[0]
    outputInput1 = vWeightArray[1,0]*hiddenOutput0 + vWeightArray[1,1]*hiddenOutput1 + biasOutputWeightArray[1]
    
    outputOutput0 = computeTransferFnctn(outputInput0, alpha)
    outputOutput1 = computeTransferFnctn(outputInput1, alpha)
    
    return (hiddenOutput0, hiddenOutput1, outputOutput0, outputOutput1)

def backpropagate(alpha, eta, errorList, actualAllNodesOutputList, inputDataList, vWeightArray, wWeightArray, 
                  biasHiddenWeightArray, biasOutputWeightArray):
    error0, error1 = errorList
    hiddenOutput0, hiddenOutput1, outputOutput0, outputOutput1 = actualAllNodesOutputList
    input0, input1 = inputDataList
    
    # Output layer derivatives
    outputDeriv0 = computeTransferFnctnDeriv(outputOutput0, alpha)
    outputDeriv1 = computeTransferFnctnDeriv(outputOutput1, alpha)
    
    # Hidden layer derivatives
    hiddenDeriv0 = computeTransferFnctnDeriv(hiddenOutput0, alpha)
    hiddenDeriv1 = computeTransferFnctnDeriv(hiddenOutput1, alpha)
    
    # Update hidden-to-output weights
    vWeightArray[0,0] += eta * error0 * outputDeriv0 * hiddenOutput0
    vWeightArray[1,0] += eta * error1 * outputDeriv1 * hiddenOutput0
    vWeightArray[0,1] += eta * error0 * outputDeriv0 * hiddenOutput1
    vWeightArray[1,1] += eta * error1 * outputDeriv1 * hiddenOutput1
    
    # Update input-to-hidden weights
    wWeightArray[0,0] += eta * hiddenDeriv0 * input0 * (vWeightArray[0,0]*error0*outputDeriv0 + vWeightArray[1,0]*error1*outputDeriv1)
    wWeightArray[1,0] += eta * hiddenDeriv1 * input0 * (vWeightArray[0,1]*error0*outputDeriv0 + vWeightArray[1,1]*error1*outputDeriv1)
    wWeightArray[0,1] += eta * hiddenDeriv0 * input1 * (vWeightArray[0,0]*error0*outputDeriv0 + vWeightArray[1,0]*error1*outputDeriv1)
    wWeightArray[1,1] += eta * hiddenDeriv1 * input1 * (vWeightArray[0,1]*error0*outputDeriv0 + vWeightArray[1,1]*error1*outputDeriv1)
    
    # Update bias weights
    biasOutputWeightArray[0] += eta * error0 * outputDeriv0
    biasOutputWeightArray[1] += eta * error1 * outputDeriv1
    biasHiddenWeightArray[0] += eta * hiddenDeriv0 * (vWeightArray[0,0]*error0*outputDeriv0 + vWeightArray[1,0]*error1*outputDeriv1)
    biasHiddenWeightArray[1] += eta * hiddenDeriv1 * (vWeightArray[0,1]*error0*outputDeriv0 + vWeightArray[1,1]*error1*outputDeriv1)
    
    return vWeightArray, wWeightArray, biasOutputWeightArray, biasHiddenWeightArray
...
def PrintAndTraceBackpropagateHiddenToInput(alpha, eta, inputDataList, errorList, actualAllNodesOutputList, transFuncDerivHiddenList, transFuncDerivOutputList, deltaWWtArray, vWeightArray, wWeightArray, newWWeightArray, biasWeightArray):
...
def main():
    ...
    try:
        maxNumIterations = int(sys.argv[1])
    ...
    while iteration < maxNumIterations: 
        ...
        print(f"SSE Total was {newSSE_Total}")  # Added for GUI communication
        sys.stdout.flush()  # Ensure output is sent to GUI
        time.sleep(0.01) 
